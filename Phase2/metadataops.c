...
Algorithm: adds op command structive with data to a linked list
...
Postcondition: assigne new structure node to beginning of linked list
               or and of established linked list
Exceptions: none
Notes: assumes memory accss/availablity
*/
OpCodeType *addNode(OpCodeType *localPtr, OpCodeType *newNode)
{
    ...
    if (localPtr == NULL)
    {
        ...
        // assign all three values to nearly created node
...
Precondition: linked list, with or withour data
...
Algorithm: iterates though op code linked list,
           display op code data invidually
Precondition: linked list, with or without data
              (should not be called if no data)
Postcondition: display all ops codes in list
Exceptions: none
Notes: none
*/
...
Precondition: provied starting index
...
int getCommand(char *cmd, char *inputStr, int index)
{
    ...
    {
        // assign charcter from input string to buffer string
...
Postcondition: return success (Boolean), returns linked list head via parameter
               return endstate/error message via parameter
...
Boolean getMetaData(char *fileName, OpCodeType **opCodeDataHead, char *endStateMsg)
{
    ...
    *opCodeDataHead = NULL;
    ...
    // allocate memory for the temporaty data structure
    // function: malloc
    ...
    // function: updataStartCount, updataEndCount
    ...
    // loop accoss all remaining op commands
    ...
    while (accessResult == COMPLETE_OPCMD_FOUND_MSG)
    {
        ...
        // updata start and end counts for later comparison
        startCount = updateStartCount(startCount, newNodePtr->strArg1);
        endCount = updateEndCount(endCount, newNodePtr->strArg1);
    }
    ...
    if (accessResult == LAST_OPCMD_FOUND_MSG)
    {
        ...
        {
            // add the last node to teh linked list
            // function: addNode
            localHeadPtr = addNode(localHeadPtr, newNodePtr);
            ...
            {
                // set access result to corrupted descripter error
                accessResult = MD_CORRUPT_DESCRIPTOR_ERR;

                // set end state message
                // function: copyString
                copyString(endStateMsg, "Metadata corrupted descropter error");
            }
        }
    }
    ...
    // checkfor any error found (not no error)
    ...
    // relase temporary structure memory
    // function:free
    free(newNodePtr);
...
Precondition: file is open and file cursor is at beginnign of an op code
Postcondition: in correct operation
                fins, tests and returns op command as parameter
...
Exceptions: correctly and appropriately (without program failure)
            ...
            incorrect or not of range op command value
Notes: none
*/
int getOpcommand(FILE *filePtr, OpCodeType *inData)
{
    ...
    // check for successfil access
    if (accessResult == NO_ERR)
    {
        // get three-letter comand
        // function: getCommand
    ...
    // otherwise, assume unsuccessfil access
    else
    {
    ...
    // get first string atg
    // function: getStringArg
    runningStringIndex = getStringArg(argStrBuffer, strBuffer, runningStringIndex);

    // set device in/out arguemnt
    // function: copyString
    ...
    // reurn complete op command found message
    return COMPLETE_OPCMD_FOUND_MSG;
}

/*
Function Name: getNumberArg
Algorithm: skips leading white steps,
            acquires next intefer from op command input string
...
int getNumberArg(int *number, char *inputStr, int index)
{
    ...
    *number = 0;
    ...
    if (foundDigt == False)
    {
        // set number to BAD_ARG_BAL constant
        *number = BAD_ARG_VAL;
    }
...
Algorithm: skip learing while space,
            acquires sub string from op command input strings
            ending ar comma or end of string
Precondition: input string has some remaing string argument
Postcondition: in correct operation,
...
{
    // function/ vairable initialization
    int localIndex = 0;
...
Precondition: test values is character
...
{
    // check for test charcter between character '0' - '9'
...
Algorithm: update number of "end" op command found in file
...
int updateEndCount(int count, char *opString)
{
    ...
    // return unchange start count
    return count;
}

/*
Function Name: updateStartCount
Algorithm: update number of "start" op command found in file
...
Algorithm: check string argument for one of the allowed string argument
Precondition: input string is provided
Postcondition: in correct operation,
                repost True if given string is in argumet list,
                False if not
Exceptions: none
Notes: none
*/
...
Algorithm: check string argument for one of the allowed command
Precondition: input string is provided
Postcondition: in correct operation,
