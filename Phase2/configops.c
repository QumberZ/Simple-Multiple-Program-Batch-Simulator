...
ConfigDataType *clearConfigData(ConfigDataType *configData)
{
    ...
    // set config data pointer to null (return as parameter)
...
Algorithm: utility function to support display
           of CPU scheduling or Lof To code strings
...
Algorithm: open file, acquires configuration data,
...
Boolean getConfigData(char *fileName, ConfigDataType **configData, char *endStateMsg)
{
    ...
    *configData = NULL;
    ...
    if (getLineTo(fileAccessPtr, MAX_STR_LEN, COLON, dataBuffer, IGNORE_LEADING_WS, dontStopAtNonPrintable) != NO_ERR || compareString(dataBuffer, "Start Simulator Configuration File") != STR_EQ)
    {
        ...
        // setend state message to corrupt leader line error
        // function: copyString
    ...
    while (lineCtr < NUM_DATA_LINES)
    {
        ...
        else
        {
            ...
            // set end state message to configuration corrupt promt error
            // function: copyString
    ...
    // function:getLineTo
    if (getLineTo(fileAccessPtr, MAX_STR_LEN, PERIOD, dataBuffer, IGNORE_LEADING_WS, dontStopAtNonPrintable) != NO_ERR || compareString(dataBuffer, "End Simulator Configuration File") != STR_EQ)
    {
        // Free temp struct memory
        // function: free
        free(tempData);
...
Algorithm: tests string for one of known leaer string, returns line number
           if string is correct, returns CFG_CORRUPT_DATA_ERR
           if string is notfound
Precondition: dataBuffer is valid C-Style string
Postcondition: return line number of data item in terms of constane
...
/*
Function Name: getLogCode
Algorithm: converts string data (e.g., "File", "Monitor")
           to constant code number to be stored as integer
Precondition: codeStr is a C-style string with one of the specified lof to operations
Postcondition: return code representing lof to action
...
Boolean valueInRange(int lineCode, int intVal, double doubleVal, char *stringVal)
{
    ...
    case CFG_MEM_AVAILABLE_CODE:
        ...
        // check for available memory limit exceeded
