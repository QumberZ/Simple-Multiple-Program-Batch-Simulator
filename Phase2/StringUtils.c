...
int compareString(const char *oneStr, const char *otherStr)
{
    ...
    // loop to end of shorter string with overrun protection
    while (oneStr[index] != NULL_CHAR && otherStr[index] != NULL_CHAR && index < MAX_STR_LEN)
    {
        ...
        // check for difference between charcter
        if (diff != 0)
        {
            // return different
            return diff;
...
void concatenateString(char *destStr, const char *sourceStr)
{
    ...
    // create temporany string pointer
    char *tempStr;
...
void copyString(char *destStr, const char *sourceStr)
{
    ...
    // check for source/destination not teh same(aliasing)
...
void setStrToLowerCase(char *destStr, const char *sourceStr)
{
    ...
    // release memory used of destination string
    // function: free
    free(tempStr);
}
